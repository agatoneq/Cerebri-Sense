...
model_path = os.path.join(base_path, "..", "files", "")
model = None  # model excluded due to scientific publication
...
def predict_patient(patient_file_path, model, X_train, scaler):
    ...
    prediction_prob = 2.0  # dummy value
    if prediction_prob < 0.1:
        ...
        classification_text = "Prediction not available – model excluded"
...
def analyze_file(file_id):
    return jsonify({
        "message": "Model temporarily unavailable due to pending scientific publication.",
        "prediction_prob": None,
        "classification_text": "Unavailable",
        "shap_values": [],
        "columns": []
    }), 503
    # try:
    ...
    #     shap_values = np.zeros_like(patient_features)  # placeholder
